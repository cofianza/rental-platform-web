 ...
 * Muestra 3 categorías: confirmar, realizar, habilitar estudio. Un item
 * es un link al panel completo `/citas` para la acción. No duplica el
 * modal de acción aquí — el usuario hace click y lo ejecuta desde el kanban.
 */
...
import { IconLoader, IconCalendar, IconClock, IconShieldCheck, IconChevronRight } from '@/components/icons'
  ...
  return (
    <div>
      ...
      <ul className="divide-y divide-gray-100 border border-gray-100 rounded-lg">
        {items.map((cita) => (
          <ItemRow key={cita.id} cita={cita} ctaLabel={ctaLabel} />
        ))}
      </ul>
    </div>
  )
...
function ItemRow({ cita, ctaLabel }: { cita: ICita; ctaLabel: string }) {
