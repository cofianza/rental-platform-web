  ...
  return (
    <div className="overflow-hidden rounded-xl border border-ink-200 bg-white transition hover:border-primary-400 hover:shadow-sm">
      ...
      <div className="flex flex-wrap gap-2 border-t border-ink-200 bg-ink-50/60 px-4 py-2.5">
        ...
        {!arrendado && !p.reservado && (
          <Link href={`/expedientes/nuevo?inmueble_id=${p.id}`} className={btnP}>
            Evaluar candidato
          </Link>
        )}
        {p.reservado && !arrendado && (
          <Link href="/contratos" className={btnP}>
            Ver el contrato
          </Link>
        )}
        {arrendado && p.contratoId && p.pago !== 'mora' && (
          <Link href={`/moras?contrato_id=${p.contratoId}`} className={btnD}>
            Reportar mora
          </Link>
        )}
        {p.pago === 'mora' && (
          <Link href="/moras" className={btnS}>
            Ver mora
          </Link>
        )}
      </div>
